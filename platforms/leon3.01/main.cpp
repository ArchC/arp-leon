...
int sc_main(int ac, char *av[])
{
  ...
  memory ram("RAM",33554432U);//32MB

  cout << "Creating Grlib APB ROM" << endl;
  memory apbrom("apbrom",4096U);//4KB

  cout << "Creating Grlib AHB ROM" << endl;
  memory ahbrom("ahbrom",4096U);//4KB

  cout << "Creating Grlib PROM" << endl;
  memory prom("prom",1048576U);//1MB

  cout << "Creating Grlib GPTIMER" << endl;
  gptimer gptimer("gptimer",50);
...
#endif
  router.MEM_port(ram.target_export);
  ...
  scanf("%s", prog);
  load_elf(leon3_cpu0, ram, prog ,RAM_BASEADDR,33554432U);
