...
//!Constructor
ac_tlm_gptimer::ac_tlm_gptimer(sc_module_name module_name, uint32_t freq):
    sc_module( module_name ),
    IRQ_port("IRQ_port", 0U),
    target_export("iport")
{
	//!Binding target export to the gptimer
   target_export( *this );
   ...
   SC_HAS_PROCESS(ac_tlm_gptimer);
   SC_METHOD(decrement_timer);
	   sensitive << clk.pos(); //System Clock
}

//!Destructor
ac_tlm_gptimer::~ac_tlm_gptimer(){}

//!Decrement Scalar clock by 1
void ac_tlm_gptimer::decrement_timer()
{
...
inline void ac_tlm_gptimer::timer_tick(){
...
//!GPTIMER underflow
inline void ac_tlm_gptimer::gptimer_hit( uint32_t id )
{
...
inline void ac_tlm_gptimer::ac_tlm_irq_send( uint32_t id )
{
	ac_tlm_req request;
	ac_tlm_rsp response;
...
ac_tlm_rsp ac_tlm_gptimer::transport( const ac_tlm_req &request ) {
...
ac_tlm_rsp_status ac_tlm_gptimer::gptimer_write(const uint32_t &a, const uint32_t &d){ 
...
ac_tlm_rsp_status ac_tlm_gptimer::gptimer_read(const uint32_t &a, uint32_t &d){ 
